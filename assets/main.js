// === Safe initialisation ===
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    const STORE_ADS_DISMISS_KEY = 'econodealStoreAdsDismissed';
    const storeAdsContainer = document.getElementById('storeAds');
    const siteDateTimeElement = document.getElementById('siteDateTime');
    const siteDateTimeFormatter = siteDateTimeElement
    ...
    function updateSiteDateTime(){
      if(!siteDateTimeElement || !siteDateTimeFormatter) return;
      const now = new Date();
      siteDateTimeElement.textContent = siteDateTimeFormatter.format(now);
      siteDateTimeElement.setAttribute('datetime', now.toISOString());
    }
    ...
    function setMenuVisibility(visible){
      ...
      if(visible){
        ...
        requestAnimationFrame(() => headerMenu.classList.add('is-visible'));
      }else{
    ...
    if(menuToggle && headerMenu){
      ...
      document.addEventListener('keydown', (event) => {
        if(event.key === 'Escape'){ setMenuVisibility(false); }
      });

      document.addEventListener('click', (event) => {
        const target = event.target;
        if(!target) return;
        if(headerMenu.contains(target) || menuToggle.contains(target)) return;
        setMenuVisibility(false);
      });
    }

    if(siteDateTimeElement && siteDateTimeFormatter){
      updateSiteDateTime();
      setInterval(updateSiteDateTime, 60000);
    }
      ...
      form.addEventListener('submit', (event) => {
        ...
        if(hasReport){
          ...
          if(!form.checkValidity()){
            alert('Veuillez fournir un nom complet et une adresse courriel valides.');
            return;
          }
    ...
    const BUREAU_EN_GROS_DEALS_API = 'api/bureau-en-gros/deals';
    ...
    function renderStoreAds(){
      if(!storeAdsContainer) return;
      const isDismissed = safeGet(STORE_ADS_DISMISS_KEY) === 'true';
      if(isDismissed){
        hideStoreAds();
        return;
      }
      ...
      const dots = items.length > 1
        ...
        : '';
      storeAdsContainer.innerHTML = `
        <button type="button" class="store-ads-dismiss" aria-label="Fermer les magasins en vedette" title="Masquer les magasins en vedette">×</button>
      ...
      const dismissBtn = storeAdsContainer.querySelector('.store-ads-dismiss');
      if(dismissBtn){
        dismissBtn.addEventListener('click', () => {
          safeSet(STORE_ADS_DISMISS_KEY, 'true');
          hideStoreAds();
        });
      }
    ...
    const BESTBUY_JSON_URL = './outputs/bestbuy/clearance.json';
    ...
    function detectRemoteDataBase(){
      ...
      if(host.endsWith('.vercel.app')) return DEFAULT_REMOTE_DATA_BASE;
      return '';
    }

    const DATA_REMOTE_BASE = detectRemoteDataBase();

    function isSportingLifePath(path){
      return typeof path === 'string'
        && path.replace(/^https?:\/\/[^/]+/, '').includes(SPORTING_LIFE_DATA_PATH);
    }
    ...
    function buildDataFetchUrls(path){
      ...
      const withQuery = (value) => query ? `${value}?${query}` : value;
      const urls = [];
      if(DATA_REMOTE_BASE){
        urls.push(withQuery(`${DATA_REMOTE_BASE}/${encoded}`));
      }
      const original = path;
      if(!urls.includes(original)) urls.push(original);
      const normalizedWithQuery = withQuery(normalized);
      if(normalizedWithQuery !== original && !urls.includes(normalizedWithQuery)) urls.push(normalizedWithQuery);
      return urls;
    }
    ...
    async function fetchDataJson(path, init){
      ...
      let lastError = null;
      const isSportingLife = isSportingLifePath(path);
      for(const url of urls){
        try{
          const options = {...(init || {})};
          if(options.cache === undefined) options.cache = 'no-store';
          if(isSportingLife){
            console.log('[Sporting Life] Fetching liquidation JSON from', url);
          }
          const res = await fetch(url, options);
          if(isSportingLife){
            console.log('[Sporting Life] Response status', res?.status, 'for', url);
          }
          if(res && res.ok) return res;
          lastError = new Error(`Request failed (${res?.status}) for ${url}`);
        }catch(err){
          if(isSportingLife){
            console.error('[Sporting Life] Fetch error for', url, err);
          }
          lastError = err;
        }
      }
      if(isSportingLife && lastError){
        console.warn('[Sporting Life] All fetch attempts failed for liquidation.json', lastError);
      }
    ...
    function render(){
      ...
      const filtered = deals.filter(d => {
        ...
        if(filterStoreSlug && storeSlug !== filterStoreSlug && !(filterStoreSlug === 'bestbuy' && storeSlug === 'best-buy')) return false;
        if(!isBestBuy && c && d.branchSlug !== c && d.citySlug !== c) return false;
      ...
      if(isDevEnv){
        ...
        const sportingLifeDeals = deals.filter(deal => slugify(deal.storeSlug || deal.store || '') === 'sporting-life');
        const sportingLifeLavalDeals = sportingLifeDeals.filter(deal => deal.branchSlug === 'laval' || deal.citySlug === 'laval');
        console.log('[DEBUG] Total deals loaded:', deals.length);
        console.log('[DEBUG] Bureau en Gros deals:', bureauDealCount);
        console.log('[DEBUG] Sporting Life deals loaded:', sportingLifeDeals.length);
        console.log('[DEBUG] Sporting Life Laval deals:', sportingLifeLavalDeals.length);
    ...
    const BEST_BUY_AGGREGATE_PATH = BESTBUY_JSON_URL;
    ...
    function filePathFor(store, branch){
      ...
      if(normalizedGroup === 'bureauengros' || s === 'bureauengros' || s === 'bureau-en-gros'){
        ...
        const encoded = encodeURIComponent(bureauSlug);
        return `${BUREAU_EN_GROS_DEALS_API}?storeSlug=${encoded}`;
      }
      if(s === 'best-buy'){
    ...
    function setCityOptions(storeSlug, preserveSelection){
      ...
      for(const branch of branches){
        ...
        const displayLabel = isNearby ? `${baseLabel} – nearby` : baseLabel;
        const attrs = [];
    ...
    async function loadBestBuyDealsFromJson() {
      const devError = document.getElementById("devError");
      try {
        const resp = await fetch(BESTBUY_JSON_URL, { cache: "no-store" });
        if (!resp.ok) {
          const msg = `[BestBuy] HTTP error ${resp.status} en lisant ${BESTBUY_JSON_URL}`;
          console.warn(msg);
          if (devError) devError.textContent = msg;
          return [];
        }

        const raw = await resp.json(); // tableau de { title, url, price, price_raw }
        if (!Array.isArray(raw) || raw.length === 0) {
          const msg = "[BestBuy] clearance.json chargé mais vide ou invalide";
          console.warn(msg);
          if (devError) devError.textContent = msg;
          return [];
        }

        const deals = raw.map((item, index) => {
          const priceNumber =
          ...
          return {
            ...
            originalPrice: null,
            discountPercent: 0, // Best Buy : on considère 0 % par défaut
            imageUrl: null,
            // Pour l’instant, on ne filtre pas par ville/branche pour Best Buy
            city: null,
            branchId: null,
          };
        });

        console.log("[BestBuy] deals chargés :", deals.length);
        if (devError) devError.textContent = `[BestBuy] ${deals.length} deals chargés`;
        return deals;
      } catch (err) {
        const msg = `[BestBuy] Erreur de chargement de clearance.json : ${err}`;
        console.error(msg);
        if (devError) devError.textContent = msg;
        return [];
      }
    ...
    async function loadDealsForStoreSlug(storeSlug){
      const store = getStoreByIdentifier(storeSlug);
      if(!store) return [];
      const seen = new Set();
      const collected = [];
      for(const branch of store.branches ?? []){
        ...
        seen.add(path);
        const deals = await fetchOne(path, store.label, branch);
        collected.push(...deals);
      }
      return collected;
    }

    const loadCanadianTireDeals = () => loadDealsForStoreSlug('canadian-tire');
    const loadBureauEnGrosDeals = () => loadDealsForStoreSlug('bureauengros');

    async function loadAllDeals() {
      const [bestBuyDeals, canadianTireDeals, bureauEnGrosDeals] = await Promise.all([
        loadBestBuyDealsFromJson(),
        loadCanadianTireDeals(),
    ...
    function normalizeBureauEnGrosItem(item){
      ...
      return {
        ...
        salePrice: item.salePrice ?? item.currentPrice ?? item.price,
    ...
    async function fetchOne(path, storeLabel, branch){
      try{
        if(path === BESTBUY_JSON_URL){
          const deals = await loadBestBuyDealsFromJson();
          return deals.map((item, index) => normalizeDeal({
            ...item,
            store: storeLabel,
            storeSlug: 'best-buy',
            branch: item.branchId || item.city || branch?.label || branch,
            url: item.productUrl || item.url,
            price: item.originalPrice ?? item.price ?? item.currentPrice,
            salePrice: item.currentPrice ?? item.price ?? item.originalPrice,
            id: item.id ?? `bestbuy-${index}`
          }, storeLabel, branch));
        }
        ...
        const prepared = storeLabel === 'Bureau en Gros'
          ? entries.map(normalizeBureauEnGrosItem)
          : entries;
        const normalized = prepared.map(item => normalizeDeal(item, storeLabel, branch));
        if(isSportingLife){
    ...
    async function fetchData(){
      ...
      if(!ALL_DEALS_CANADA.length){
        ALL_DEALS_CANADA = await loadAllDeals();
      }

      deals.length = 0;
      const storeFilter = slugify(storeSelect.value || '');
      const branchFilter = citySelect.value;
      const postalFilter = postalBranchSlugs && postalBranchSlugs.size ? postalBranchSlugs : null;

      for(const deal of ALL_DEALS_CANADA){
        const storeSlug = slugify(deal.storeSlug || deal.store || '');
        const isBestBuy = storeSlug === 'best-buy' || storeSlug === 'bestbuy';

        if(storeFilter && storeSlug !== storeFilter && !(storeFilter === 'bestbuy' && storeSlug === 'best-buy')){
          continue;
        }

        if(!isBestBuy){
          if(branchFilter && deal.branchSlug && deal.branchSlug !== branchFilter && deal.citySlug !== branchFilter){
            continue;
          }
          if(postalFilter && deal.branchSlug && !postalFilter.has(deal.branchSlug)){
            continue;
          }
        }
    ...
    range.value = DEFAULT_FILTERS.discount; // ← 0% by default
    await fetchData();
  });
