...
import {
  ...
  readBureauEnGrosDealsForStore as readBureauEnGrosDealsForStoreFromDisk,
} from './bureauEngros';

export async function readBureauEnGrosDealsForStore(storeSlug) {
  const res = await fetch(`/bureauengros/${storeSlug}/data.json`);
  if (!res.ok) {
    throw new Error(
...
export async function loadBureauEnGrosDeals({ store, minDiscount = 0 }) {
  try {
    ...
    const products = store
      ? readBureauEnGrosDealsForStoreFromDisk(store)
      : readBureauEnGrosDealsForAllStores();
