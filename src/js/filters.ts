import { createCurrencyFormatter, createPercentFormatter } from './utils/storage';

const DEFAULT_BASE_CURRENCY = { code: 'CAD', locale: 'fr-CA' };

export async function initFilters(options = {}) {
    const postalMessages = options.postalMessages;
    const BASE_CURRENCY = options.baseCurrency ?? DEFAULT_BASE_CURRENCY;
  ...
  const calculatorCurrencyNodes = Array.from(document.querySelectorAll('[data-calculator-currency]'));
  const profitInputs = profitCalculator ? {
    purchase: profitCalculator.querySelector('[data-field="purchase"]'),
    sale: profitCalculator.querySelector('[data-field="sale"]'),
    shipping: profitCalculator.querySelector('[data-field="shipping"]'),
    feePercent: profitCalculator.querySelector('[data-field="feePercent"]'),
    extras: profitCalculator.querySelector('[data-field="extras"]')
  } : null;
  const profitOutputs = profitCalculator ? {
    profit: profitCalculator.querySelector('[data-profit-amount]'),
    margin: profitCalculator.querySelector('[data-profit-margin]'),
    fees: profitCalculator.querySelector('[data-marketplace-fees]'),
    totalCost: profitCalculator.querySelector('[data-total-cost]'),
    breakEven: profitCalculator.querySelector('[data-break-even]')
  } : null;
  let calculatorCurrencyFormatter = createCurrencyFormatter(BASE_CURRENCY.locale, BASE_CURRENCY.code);
  let calculatorPercentFormatter = createPercentFormatter(BASE_CURRENCY.locale);
...
function formatPercentValue(value){
  ...
  try{
    return calculatorPercentFormatter.format(normalized);
  }catch(_err){
...
function updateProfitCalculator(){
  if(!profitCalculatorForm || !profitInputs || !profitOutputs) return;
  ...
  if(profitInputs.feePercent && typeof profitInputs.feePercent.max === 'string' && profitInputs.feePercent.max){
    const maxAttr = Number.parseFloat(profitInputs.feePercent.max);
    if(Number.isFinite(maxAttr) && feePercent > maxAttr){
      feePercent = maxAttr;
    }
  }
  ...
  if(profitOutputs.margin){
    ...
    const sign = sale > 0 ? (displayMargin > 0 ? '+' : displayMargin < 0 ? '−' : '') : '';
  ...
  if(profitOutputs.breakEven){
    if(Number.isFinite(breakEven) && breakEven >= 0){
      ...
      profitOutputs.breakEven.textContent = '—';
...
function fullBestBuyBranches(){
  return [
    ...
    {label:'Bay & Dundas', slug:'bay-dundas-977', city:'Toronto', address:'65 Dund, Toronto, ON', storeNumber:'977', hasData:false},
    ...
    {label:'Best Buy Mobile Mayfair Shopping Centre', slug:'best-buy-mobile-mayfair-shopping-centre-239', city:'Victoria', address:'3147 Dougl, Unit 344, Victoria, BC', storeNumber:'239', hasData:false},
...
const DEFAULT_BRANCH_SLUGS = new Set([
  ...
  'montreal-best-buy',
  'laval-best-buy',
  'laval-2075-bd-chomedey',
  'laval-chomedey',
  'laval-robert-bourassa'
]);
...
for(const branch of baseBranches()){
  if(branch.slug === 'laval'){
    CANADIAN_TIRE_BRANCHES.push({
      ...branch,
      label: 'Laval — 1450 Boul. le Corbusier',
      address: '1450 Boul. le Corbusier, Laval, QC',
      slug: 'laval',
      hasData: true
    });
    CANADIAN_TIRE_BRANCHES.push({
      ...branch,
      label: 'Laval — 4975 Blvd. Robert-Bourassa',
      address: '4975 Blvd. Robert-Bourassa, Laval, QC',
      slug: 'laval-robert-bourassa',
      hasData: true
    });
    CANADIAN_TIRE_BRANCHES.push({
      ...branch,
      label: 'Laval — 2075 Bd Chomedey',
      address: '2075 Bd Chomedey, Laval, QC',
      slug: 'laval-chomedey',
      hasData: true
    });
    continue;
...
const DEFAULT_POSTAL_MESSAGE = postalMessages.defaultMessage;
...
async function fetchDataJson(path, init){
  ...
  for(const url of urls){
    try{
      ...
      lastError = new Error(`Requête échouée (${res?.status}) pour ${url}`);
    }catch(err){
      lastError = err;
    }
  }
...
// currency.cadToLocalRate indique la conversion d'un dollar canadien vers la devise locale.
  const COUNTRY_CONFIG = { ...DEFAULT_COUNTRY_CONFIG };
  const overrides = options.countryConfig || {};
  for (const [country, config] of Object.entries(overrides)) {
    const existing = COUNTRY_CONFIG[country] || DEFAULT_COUNTRY_CONFIG.canada;
    COUNTRY_CONFIG[country] = {
      ...
      storeMenu: config.storeMenu || existing.storeMenu,
    };
  }
...
async function applyPostalSearch(raw){
  ...
  if(normalized.length < 3){
    ...
    updatePostalHelper(postalMessages.incomplete, true);
    if(hadFilter){
      postalBranchSlugs = null;
      setCityOptions(storeSelect?.value ?? '', true);
      if(citySelect){
  ...
  if(!match){
    postalInput.setAttribute('aria-invalid','true');
    updatePostalHelper(postalMessages.unknown, true);
    if(hadFilter){
      postalBranchSlugs = null;
      setCityOptions(storeSelect?.value ?? '', true);
      if(citySelect){
