...
async def fetch_and_extract(proxy: Optional[Dict[str, Optional[str]]]):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for sel in ["button:has-text('Accepter')", "button:has-text('OK')", "button[aria-label='close']"]:
...
async def main():
    ...
    if not PROXIES:
        raise RuntimeError("No proxies configured. Set PROXY*_SERVER secrets or update DEFAULT_PROXIES.")
