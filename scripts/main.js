...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const yearFooterNode = document.getElementById('yearFooter');
  if(yearFooterNode){
    yearFooterNode.textContent = currentYear;
  }
  ...
  let calculatorCurrencyFormatter = new Intl.NumberFormat('fr-CA',{style:'currency',currency:'CAD'});
  let calculatorPercentFormatter = new Intl.NumberFormat('fr-CA',{maximumFractionDigits:1,minimumFractionDigits:0});
  ...
  const hasSearchUI = Boolean(storeSelect && citySelect && range && rangeLabel && cardsEl && countEl && btnClear && postalInput && postalHelper);
  if(!hasSearchUI){
    return;
  }
